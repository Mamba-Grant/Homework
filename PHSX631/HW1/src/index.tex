\begin{homeworkProblem}
    ...
    \begin{callout}{Solution, part (a):}
        ...
        \begin{center}
            \includegraphics[width=0.25\textwidth]{../mydrawing1.png}
            \includegraphics[width=0.25\textwidth]{../mydrawing2.png}
            \includegraphics[width=0.25\textwidth]{../mydrawing3.png}
        \end{center}
    ...
    \begin{callout}{Solution, part (b):}
        ...
        \begin{align*}
            B(z) &= \frac{\mu_0 M a}{4\pi} \int_0^{2\pi} \int_{-L/2}^{L/2} \frac{d\phi' dz'}{(a^2 + (z-z')^2)^{3/2}} \\
            &= \frac{\mu_0 M a}{2} \int_{-a}^{a} \frac{dz'}{(a^2 + (z-z')^2)^{3/2}} \\ 
            &= -\frac{\mu_0 Ma}{2}\int_{z-a}^{z+a} \frac{u}{\left( a^2 + u^2 \right)^{3/2}} \,du &&\begin{matrix} u = z-z' \\ du = -dz' \end{matrix} \\ 
            &= -\frac{\mu_0 Ma}{2}\int_{z-a}^{z+a} \frac{a \sec ^2 \theta }{\left( a^2 \sec^2\theta \right)^{3/2}} \,d\theta &&\begin{matrix} u = a\tan\theta \\ du = a\sec^2\theta \end{matrix} \\
            &= -\frac{\mu_0 M}{2a}  \left( \sin\theta \right) \bigg|_{z-a}^{z+a} \\ 
            &= \frac{\mu_0 M}{2a} \left( \frac{z-a}{\sqrt{a^2 + (z-a)^2}} - \frac{z+a}{\sqrt{a^2 + (z+a)^2}}\right) &&\sin\theta = \frac{u}{\sqrt{a^2+u^2}}
        \end{align*}

        Great, I have $B(z)$. Now $H(z)$ is 
        $$
        H(z) = \frac{B}{\mu_0} - M(z) = 
        \begin{cases}
            \frac{B(z)}{\mu_0} - M, & -a<z<a \\ 
            \frac{B(z)}{\mu_0}, & \text{elsewhere.}
        \end{cases}
        $$
        \textit{(Since $M(z) = M$ inside, and 0 outside.)}
        A plot of this is 

        \begin{center}
        \begin{tikzpicture}[
    declare function={
        B(\z,\a) = 1/2 * ((\z-\a)/(sqrt(\a*\a + (\z-\a)*(\z-\a))) - 
                          (\z+\a)/(sqrt(\a*\a + (\z+\a)*(\z+\a))));
        M(\z,\a) = and(\z >= -\a, \z <= \a) * 1;
        H(\z,\a) = B(\z,\a) - M(\z,\a);
    }
]
    ...
    \begin{axis}[
        ...
        % Plot B(z)
        \addplot[thick, blue] {B(x,1)};
        \addlegendentry{$B(z)/\mu_0M$}
        
        % Plot H(z)
        \addplot[thick, green!60!black] {H(x,1)};
        \addlegendentry{$H(z)/M$}
        
        % Plot M(z) - using two separate plots for the discontinuity
        \addplot[thick, red, domain=-1:1] {1};
        \addplot[thick, red, domain=-4:-1] {0};
        \addplot[thick, red, domain=1:4] {0};
        \addlegendentry{$M(z)/M$}
        
        % Add vertical lines at z = ±a
        \addplot[dashed, gray, thick] coordinates {(-1,-3) (-1,3)};
        \addplot[dashed, gray, thick] coordinates {(1,-3) (1,3)};
    \end{axis}
\end{tikzpicture}
\end{center}

And I believe this makes sense: the magnetic field should converge to zero at large $z$, and we would expect a higher magnetic field \textit{magnitude} inside the magnetic domain.
