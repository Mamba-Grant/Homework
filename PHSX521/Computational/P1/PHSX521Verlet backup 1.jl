...
Write a code to solve $\ddot{\phi} = -\frac{g}{R} \sin \phi$, using the Velocity Verlet algorithm, using the algorithm described in class. Note that we already have an equation for $\dot{\phi}$, so no need to calculate forces first. Assume that $R = 5 \, \text{m}$ and $g = 9.8 \, \text{m/s}^2$.
...
md"
The animation will also be saved to newton.mp4 in the same directory as the code
"
...
# ╔═╡ 918c353f-4162-4b56-b722-1783e131212e
fig
...
# ╔═╡ 879d9476-19ce-4eda-926a-6932c047a508
...
# ╔═╡ 4dbdec94-4c65-4dc7-bde1-da55bd7d4067
begin
	# Create a slider to control the frame range interactively
	@bind frame Slider(1:120)
	
	# Initialize the canvas and axes, and define the first animation
	fig, ax = Figure(resolution = (600, 600))
	points = Point2f0[]
	colors = Int[]
	
	# Define the second animation's points and colors
	points2 = Point2f0[]
	colors2 = Int[]
	
	# Create the first lines plot with inferno colormap
	lineplot1 = lines!(ax, points, color = colors, colormap = :inferno, transparency = true)

	Observable(points)[] = [step!(i) for i in 1:frame]
	Observable(colors)[] = collect(1:frame)
	lineplot1.colorrange = (0, frame)
end
...
manifest_format = "2.0"
project_hash = "9cbd5cdf455a0ab7e902ddefb50024b06cd62a9a"
