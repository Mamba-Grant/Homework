...
# ╔═╡ 86230637-7bca-40b9-a123-f555c24110bc
using Plots # Standard plotting library

# ╔═╡ ca5a6cc8-88d2-4b61-b932-d9c605e75e64
# There are many excellent libraries for this in julia, this is just the first I saw. It is pretty neat that it compiles to utilise the GPU. Thanks LLVM :)
using DifferentialEquations

# ╔═╡ 4798ded0-b2c7-11ef-3714-abfc87bd54b3
md"# Computational Project: Part 3

**Instructor:** Hartwin Peelaers  
**Course:** PHSX521/EPHX521 Fall 2024  
**Due:** Wednesday 12/11, 11:59PM (submitted by email to [peelaers@ku.edu](mailto:peelaers@ku.edu))  

---

This project counts for **20% of your final grade**, and this is part 3/3. Grading will be based on:
- **50%**: Having a working code.
- **20%**: Commenting the code (always a good practice).
- **30%**: Generating useful figures and explaining them and the physics they contain.

### Guidelines:
- You can use any programming or scripting language (Python, C, C++, Fortran, Matlab, etc.).
- For compiled languages (e.g., C++), write data to a file and use an external program for plots (e.g., gnuplot, xmgrace, matplotlib).
- Optimized code is not required.
- For figures:
  - Label the axes.
  - Use a legend when necessary.
- If using Python:
  - You can submit a Jupyter notebook containing code, figures, answers, and explanations.
  - Alternatively, use Google Colab ([colab.research.google.com](https://colab.research.google.com)) and ensure access is granted or submit the notebook file.

### Important:
- **Start early!** Coding and debugging take time. Questions can be addressed during office hours.
- **Early submission bonus:** Submissions before **Monday 12/09, noon** will be reviewed for missing or incorrect elements, allowing corrections until the deadline. Only the final submission will be graded.

---

## Problems:

### 1. **Simulate a Damped Harmonic Oscillator**
   - **Parameters:** $k = m = \omega = 1$
   - Use the Velocity Verlet algorithm.

   #### Tasks:
   (a) Explore different damping regimes:
   - No damping.
   - Underdamping.
   - Critical damping.
   - Overdamping.
   ...
   **Output:**
   ...
   - Plot mechanical energy as a function of time for each damping regime.
   - Ensure each regime is identifiable.

   *Note:* Distinguish between $b$ and $\beta$ and use the correct values.

   (b) Extend the simulation by adding a driving force $\cos(\omega t)$:
   - Test different damping values ($\beta$) and driving frequencies ($\omega$):
     - Far from resonance.
     - Near/at resonance.
   - Use a sufficiently large simulation time to eliminate transient solutions.

   **Discuss the results.**

---

### 2. **Solve Differential Equation with a Library**
   - Differential equation: $\ddot{\phi} = -\frac{g}{R} \sin\phi$
     - $R = 5 \, \text{m}, \, g = 9.8 \, \text{m/s}^2, \, \phi_0 = \frac{\pi}{2}$.

   #### Tasks:
   - Use a library to solve the equation (e.g., `scipy.integrate.solve_ivp` in Python or `Boost.Numeric.Odeint` in C++).
   - Plot results from:
     1. Library method.
     2. Velocity Verlet algorithm.

   **Output:** Combine both results in the same plot.  
   *Note:* Some libraries only handle first-order equations; rewrite the second-order equation as two first-order equations if needed.
"
...
# ╔═╡ 13653395-6543-4e0a-98b7-93760c1661db
md"I'd like to initialize some global variables for timestep and time stop:"

# ╔═╡ 2a7218e3-46c8-446b-b1d7-14c23da97af4
begin
	dt = 1e-3
	time_stop = 40
	iterations = 1/dt * time_stop # (conversion * seconds)
end

# ╔═╡ a823a582-1af6-40f7-a533-9af747b8f855
md"now I initialize my systems:"

# ╔═╡ 0c6ed505-7b02-4b73-a570-786d776391e1
begin
...
# ╔═╡ be4fb2d2-e522-416b-9ea7-3ce1cfec540e
md"I was very sloppy with my code last time, so instead this time I will just have a list of my simulations, stepping through them one at a time and appending their points to lists. It would be better practice to make this a struct so I could access each array of points by name, rather than an arbitrary index, but this is good enough.
"

# ╔═╡ 02e6d80e-c7e6-449c-bc0e-1be4d5614f0e
begin
	points = [Float64[] for system in systems]
	mechanical_energies = [Float64[] for system in systems]
end

# ╔═╡ ce1cf975-50e9-4356-8626-bcb9cf5614f1
for (i, system) in enumerate(systems)
    for t in 1:iterations
        step!(system)
        push!(points[i], system.x)
		push!(mechanical_energies[i], 0.5*system.dx^2 + 0.5*system.k*system.x^2)
    end
end
...
begin
	plots = [
		...
		legend=:outerright, yrange=[-1,1]),
	    plot(dt:dt:time_stop, points[4], label="Overdamping", color=:purple,
		legend=:outerright, yrange=[-1,1])
	]
...
md"
*(I apologize for my lack of axis labels, I wasn't able to work out how to set a global one, since by default Plots likes to stick labels on each sub plot making everything illegible)*

- The spring without damping exhibits pure sinusoidal motion that doesn't decay.
- The underdamped spring oscillates with a gradually decreasing amplitude over time.
- The critically damped spring returns to equilibrium as quickly as possible without oscillating.
- The overdamped spring returns to equilibrium slowly, without oscillating.
"

# ╔═╡ 82edb9c8-9972-4fa4-8bcb-4e0736058b41
begin
	energy_plots = [
	    plot(dt:dt:time_stop, mechanical_energies[1], label="No Damping", color=:blue, legend=:outerright, yrange=[-1,1], title="Comparison of Mechanical Energy"),
	    plot(dt:dt:time_stop, mechanical_energies[2], label="Underdamping", color=:red, legend=:outerright, yrange=[-1,1]),
	    plot(dt:dt:time_stop, mechanical_energies[3], label="Critical Damping", color=:green, legend=:outerright, yrange=[-1,1]),
	    plot(dt:dt:time_stop, mechanical_energies[4], label="Overdamping", color=:purple, legend=:outerright, yrange=[-1,1])
	]
	plot(energy_plots..., layout=layout)
end

# ╔═╡ fd428937-f865-4484-a7eb-fb5fc4f73c01
md"
- The spring with no damping exhibits constant energy, indicating that it is prefectly conservative. 
- The underdamped spring decays slowly with time, close to exponentially.
...
md"
# Part 2. Solving differential equations with a library
"

# ╔═╡ bf2ece70-5475-46c0-9085-0320d1c02d82
# a tiny bit of code to get custom tick marks on the plot in terms of π
# https://discourse.julialang.org/t/plot-with-x-axis-in-radians-with-ticks-at-multiples-of-pi/65325/3
...
# ╔═╡ 07a2e8cc-4d5d-4e95-94b5-cf1954fa3c67
begin
	# Define the system parameters
	g = 9.8 # gravitational constant (m/s²)
	R = 5.0 # pendulum length (m)
	
	# Initial conditions
	φ₀ = π/2     # initial angle (45°)
	ω₀ = 0.0     # initial angular velocity
	u₀ = [φ₀, ω₀]
end

# ╔═╡ 8155a9de-5e35-4290-bc2d-0e944fd31850
function pendulum!(du, u, p, t)
	φ, ω = u    # u[1] = φ (angle), u[2] = ω (angular velocity)
	
	# Convert second-order ODE to system of first-order ODEs 
	# (a quirk of this particular library)
	du[1] = ω
	du[2] = -(g/R) * sin(φ)
end

# ╔═╡ 91bbdd6f-f7cf-490a-bfe9-8633655ed3bd
begin
	prob = ODEProblem( pendulum!, u₀, (0, time_stop) )
	sol = solve(prob, Tsit5())
end

# ╔═╡ 88e07a67-4c27-4a0a-b2ba-904d64371e50
md"Now, to set up a verlet solver:"

# ╔═╡ b95f74ad-330d-453e-b585-d0968f228374
# Structure to hold pendulum state and parameters
Base.@kwdef mutable struct Newtonian2
	dt::Float64 = 1e-3
    x::Float64 = π/2
    dx::Float64 = 0.0
    ddx::Float64 = 0.0
...
# ╔═╡ 075a121e-e21e-4579-9b61-48104359339d
function step_pendulum!(n::Newtonian2)
	ddx = -(n.g/n.R) * sin.(n.x)
    # Update position using the previous velocity and acceleration
    n.x = @. n.x + n.dt * n.dx + (n.dt^2) / 2 * n.ddx
...
# ╔═╡ d890b65b-6d3a-46a2-a4a5-9980ca2226f6
begin
	points_pendulum = Float64[]
	system_pendulum = Newtonian2()

	for t in 1:iterations
        step_pendulum!(system_pendulum)
        push!(points_pendulum, system_pendulum.x)
    end
end

# ╔═╡ 6e277b43-706f-4dab-bb6d-cefa307eb7a9
begin
	plot(dt:dt:time_stop/2, [φ[1] for φ in sol.(dt:dt:time_stop/2)], 
		ytick=pitick(-π/2, π/2, 6),
		xlabel="Time (s)", 
		ylabel="Angle φ (rad)",
		title="Simple Pendulum Motion",
		label="φ(t), DifferentialEquations lib",
		legendposition=:topright,
		linewidth=3)
	
	plot!(dt:dt:time_stop, points_pendulum, color=:green, ls=:dash, linewidth=3, label="φ(t), Verlet",xrange=[0,20])
end
