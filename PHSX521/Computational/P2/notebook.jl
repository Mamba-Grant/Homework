...
# ╔═╡ 7531bf38-060c-4fad-a913-caab665dab81
using Plots
...
Base.@kwdef mutable struct Newtonian
	dt::Float64 = 0.001
	g::Float64 = 9.8
	D::Float64 = 15e-2 # units m
	m::Float64 = (4/3*π*(D/2)^3) * (7.8e3) # units kg (~13.78 kg)
	r::Tuple{Float64, Float64} = (0, 0) # units m
	dr::Tuple{Float64, Float64} = (0, 0) # units m/s
	ddr::Tuple{Float64, Float64} = (0, 0) # units m/s^2
	γ::Float64 = 0.25 # units Ns^2/m^4
...
begin
	...
	x_positions_with_air = Float64[]
	y_positions_with_air = Float64[]
	x_velocity_with_air = Float64[]
	y_velocity_with_air = Float64[]
	times = Float64[]
	t = 0.0
	
	# Collect data points by stepping through simulation
	for i in 1:tstop
		step!(system_with_air)
		push!(x_positions_with_air, system_with_air.r[1])
		push!(y_positions_with_air, system_with_air.r[2])
		push!(x_velocity_with_air, system_with_air.dr[1])
		push!(y_velocity_with_air, system_with_air.dr[2])
		push!(times, t)
		t += dt
	end
	
	# Initialize system without air resistance  
    system_no_air = Newtonian(dr=(300*cosd(50),300*sind(50)), dt=dt, c=0)
    
    # Initialize vectors to store positions
    x_positions_no_air = Float64[]
    y_positions_no_air = Float64[]
	x_velocity_no_air = Float64[]
	y_velocity_no_air = Float64[]
    
    # Collect data points by stepping through simulation
    t = 0.0
    for i in 1:tstop
        step!(system_no_air)
        push!(x_positions_no_air, system_no_air.r[1])
        push!(y_positions_no_air, system_no_air.r[2])
		push!(x_velocity_no_air, system_no_air.dr[1])
		push!(y_velocity_no_air, system_no_air.dr[2])
        t += dt
    end
    ...
    y_positions_no_density = Float64[]
	x_velocity_no_density = Float64[]
	y_velocity_no_density = Float64[]
    
    # Collect data points by stepping through simulation
    t = 0.0
    for i in 1:tstop
        step!(system_no_density)
        push!(x_positions_no_density, system_no_density.r[1])
        push!(y_positions_no_density, system_no_density.r[2])
		push!(x_velocity_no_density, system_no_density.dr[1])
		push!(y_velocity_no_density, system_no_density.dr[2])
        t += dt
    end
end

# ╔═╡ 29dc7f8d-e7df-424f-94c0-47e69022344a
begin
    ...
    plot!(x_positions_no_air, y_positions_no_air,
          ...
          color=:red)
    plot!(x_positions_no_density, y_positions_no_density,
    ...
    p2 = plot(times, x_positions_with_air, label="With Air Resistance", color=:blue,
              ...
              ylabel="Distance (m)",
              legend=false)
    plot!(times, x_positions_no_air, label="Without Air Resistance", color=:red, legend=false)
    plot!(times, x_positions_no_density, label="Without Changing Atmospheric Density", color=:green, legend=false)
    
    p3 = plot(times, y_positions_with_air, label="With Air Resistance", color=:blue,
              title="Y vs Time", 
              xlabel="Time (s)",
              ylabel="Height (m)",
              legend=false)
    ...
    p4 = plot(times, x_velocity_with_air, label="With Air Resistance", color=:blue,
              title="dx/dt vs Time", 
              xlabel="Time (s)",
              ylabel="Velocity (m)",
              legend=false)
    plot!(times, x_velocity_no_air, label="Without Air Resistance", color=:red, legend=false)
    plot!(times, x_velocity_no_density, label="Without Changing Atmospheric Density", color=:green, legend=false)
    
    p5 = plot(times, y_velocity_with_air, label="With Air Resistance", color=:blue,
              title="dy/dt vs Time", 
              xlabel="Time (s)",
              ylabel="Velocity (m)",
              legend=false)
    plot!(times, y_velocity_no_air, label="Without Air Resistance", color=:red, legend=false)
    plot!(times, x_velocity_no_density, label="Without Changing Atmospheric Density", color=:green, legend=false)
