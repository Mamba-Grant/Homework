...
\begin{callout}{My preferred solution, 10.7 from the calculus of variations (MATH 648):}
    ...
    which implies a convex function assuming smoothness. The aboce theorem then implies the curves which optimize the functional are straight lines.
...
\begin{callout}{Solution Attempt \#1:}
    We have 
    \begin{align*}
        T &= \begin{pmatrix} \frac{1}{2}m \frac{d}{dt}\left( R\cos(\omega t ) + l\sin\theta \right)^2 \\ \frac{1}{2}m \frac{d}{dt}\left( R\sin(\omega t ) - l\cos\theta \right)^2 \end{pmatrix}
            = \begin{pmatrix} \frac{1}{2}m \left( R \omega \cos(\omega t ) + l \dot{\theta}\sin\theta \right)^2 \\ \frac{1}{2}m \left( R \omega \sin(\omega t ) - l \dot{\theta}\cos\theta \right)^2 \end{pmatrix}\\
        V &= mgh = mg \left( R\sin(\omega t ) - l\cos\theta \right)
    \end{align*}
    So then we have a functional with fixed endpoints, and a specified $\theta(t=0) = 0$.
    \begin{align*}
        L = 
        \frac{1}{2}m
        \begin{pmatrix} 
            R^2\omega^2\cos^2(\omega t)+2\omega lR \dot{\theta} \cos (\omega t)\sin (\theta)+l^2 \dot{\theta}^2\sin^2(\theta) \\ 
            R^2\omega^2\sin^2(\omega t)-2\omega lR \dot{\theta} \sin (\omega t)\cos (\theta)+l^2 \dot{\theta}^2\cos^2(\theta)
        \end{pmatrix}
        - mg [R\sin(\omega t ) - l\cos\theta] 
    \end{align*}
    Really, for kinetic energy, $\textbf{v}^2 = v_x^2 + v_y^2$, despite me writing it out as I did in a vector. This allows us to use our beloved trigonometic identities to simplify.
    \begin{align*}
        J &= \int_{t_0}^{t_1} \frac{1}{2}m \left[ R^2 \omega^2 + l^2 \dot{\theta}^2 + 2\omega lR \dot{\theta} \cos (\omega t)\sin (\theta) - 2\omega lR \dot{\theta} \sin (\omega t)\cos (\theta) \right] - mg [R\sin(\omega t ) - l\cos\theta] ~d\theta
    \end{align*}
    There are no special cases to be applied, so the ordinary E-L applies:
\begin{align*}
    0 &= \frac{m}{2} \frac{d}{dt}\left( 2 l^2 \dot{\theta} + 2 \omega l R \cos(\omega t)\sin(\theta)- 2 \omega l R \dot{\theta}\sin(\omega t)\cos(\theta) \right) \\ 
    &- m\left[2 \omega l R \dot{\theta}(\cos (\omega t) \cos (\theta)+\sin (\omega t) \sin (\theta))-l^2 \dot{\theta}^2 \cos (\theta)+g l \sin (\theta)\right] \\
    0 &= m \left( l^2 \ddot{\theta} + \omega l R\left[ - \omega \sin ( \omega t)\sin (\theta)+\dot{\theta} \cos( \omega t)\cos (\theta)\right]- \omega l R \left[ \omega\cos(\omega t)\cos(\theta) - \dot{\theta}\sin(\omega t)\sin(\theta) \right] \right) \\ 
    &- m\left[2 \omega l R \dot{\theta}(\cos (\omega t) \cos (\theta)+\sin (\omega t) \sin (\theta))-l^2 \dot{\theta}^2 \cos (\theta)+g l \sin (\theta)\right]
\end{align*}
\end{callout}
\begin{callout}{Solution Attempt \#2:}
    
    This is quite horrendous, and no amount of simplifying will get a differential equation that could be solved. 
    Instead I will try to reframe the problem with a new coordinate system. 
    If the coordinate system is rotating about the origin $O$ at a rate $\omega$, I should be able to eliminate explicit time dependence. 
    Kinetic energy has translational and rotational parts:
    \begin{align*}
        T &= \frac{1}{2}m \left( l^2\dot{\theta} + R^2 \omega^2 + 2Rl \dot{\theta}\omega \cos(\theta) \right)
    \end{align*}
